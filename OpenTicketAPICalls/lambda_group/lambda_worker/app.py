...
from table_helper import create_df
...
def lambda_handler(event, context):
    try:
        ...
        logger.exception(
            "Failed"
        )  # logger.exception automatically includes trackback (only inside except block)
        raise  # report a failure to CloudWatch and EventBridge


if __name__ == "__main__":
    lambda_handler({"Records": [{"body": '{"duns":"078430362"}'}]}, None)
